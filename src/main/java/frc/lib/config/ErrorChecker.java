...
public class ErrorChecker {
     ...
     * When we configure our devices, we wrap all our our calls to the devices in
     * this method
     ...
     * config.configureSettings() until there is a successful configuration or until
     * it times out.
     * If printInfo is true, it will print if the configuration succeeded and how
     * many tries it took
     */
    public static void configureDevice(DeviceConfiguration config, String name, double bootAllowanceSeconds,
            boolean printInfo) {
            ...
            if (timer.get() > bootAllowanceSeconds) {
                if (printInfo)
                    DriverStation.reportError(
                            "failed configuration for " + name + " initialization, took " + tries + " tries.", false);
                return;
            }
        ...
        if (printInfo && tries > TRIES_TO_GENERATE_WARNING)
            DriverStation.reportWarning("Possible issue with " + name + ". Configuration took " + tries + " tries",
                    false);
        else if (printInfo)
            System.out.println("[Config] " + name + " | configuration took " + tries + " tries. ");
    }
