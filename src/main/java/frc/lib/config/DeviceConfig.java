...
public class DeviceConfig {
    ...
    public static TalonFXConfiguration steerFXConfig(int remoteSensorID) {
        ...
        config.Feedback = FXSteerFeedbackConfig(FeedbackSensorSourceValue.FusedCANcoder, remoteSensorID,
                SteerConstants.GEAR_RATIO, Rotation2d.fromRotations(0));
    ...
    public static void configureTalonFX(String name, TalonFX motor, TalonFXConfiguration config,
        ...
        ErrorChecker.configureDevice(deviceConfig, name + " " + motor.getDeviceID() + " version " + motor.getVersion(),
                true);
    }
    ...
    public static void configureSwerveEncoder(String name, CANcoder encoder, CANcoderConfiguration config,
        ...
        ErrorChecker.configureDevice(deviceConfig,
                name + " " + encoder.getDeviceID() + " version " + encoder.getVersion(), true);
    }

    public static void configurePigeon2(String name, Pigeon2 pigeon, Pigeon2Configuration config,
            double updateFrequencyHz) {
        DeviceConfiguration deviceConfig = new DeviceConfiguration() {
            @Override
            public boolean configureSettings() {
        ...
        ErrorChecker.configureDevice(deviceConfig,
                name + " " + pigeon.getDeviceID() + " version " + pigeon.getVersion(), true);
    }
    ...
    public static FeedbackConfigs FXFeedbackConfig(FeedbackSensorSourceValue sensor, int remoteSensorID,
            double sensorToMechGR, Rotation2d sensorOffset) {
    ...
    public static FeedbackConfigs FXSteerFeedbackConfig(FeedbackSensorSourceValue sensor, int remoteSensorID,
            double rotorToSensorGR, Rotation2d sensorOffset) {
        FeedbackConfigs config = new FeedbackConfigs();
        config.FeedbackSensorSource = sensor;
        config.FeedbackRemoteSensorID = remoteSensorID;
    ...
    public static CurrentLimitsConfigs FXCurrentLimitsConfig(boolean enable, double limit, double currentThreshold,
    ...
    public static TorqueCurrentConfigs FXTorqueCurrentConfig(double peakForward, double peakReverse,
