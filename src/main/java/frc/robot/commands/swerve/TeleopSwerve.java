...
public class TeleopSwerve extends Command {
     ...
     * @param fieldRelativeSup A supplier for the drive mode. Robot relative =
     *                         false; Field relative = true
     */
    public TeleopSwerve(Swerve swerve, DoubleSupplier[] translationSup, DoubleSupplier rotationSup,
            BooleanSupplier fieldRelativeSup) {
        this.swerve = swerve;
     ...
     * Passes the translation, strafe, and rotation values to the swerve subsystem
     * to drive the robot.
     */
    @Override
    public void execute() {

        Translation2d translationVal = new Translation2d(translationSup[0].getAsDouble(),
                translationSup[1].getAsDouble()).times(SwerveConstants.MAX_SPEED);
        double rotationVal = getRotation(rotationSup.getAsDouble());
        boolean fieldRelativeVal = fieldRelativeSup.getAsBoolean();

        if (Controlboard.getZeroGyro().getAsBoolean())
            lastAngle = Rotation2d.fromDegrees(0.0);

        swerve.setChassisSpeeds(
                fieldRelativeVal ? swerve.fieldRelativeSpeeds(translationVal, rotationVal)
                        : swerve.robotRelativeSpeeds(translationVal, rotationVal),
                true);
    }
     ...
     * <p>
     * If no manual input is given for a time, this method will return the rotation
     * value required to maintain the current heading.
     * 
     * @param current The current rotation value.
     * @return The determined rotation value.
     */
    private double getRotation(double current) {
        boolean rotating = Math.abs(current) > 0.01;
